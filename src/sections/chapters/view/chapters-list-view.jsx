...
import ChaptersTableRow from '../chapters-table-row';


const TABLE_HEAD = [
  { id: 'index', label: 'Serial No' },
  { id: 'chapter_name', label: 'Chapter Name' },
  { id: 'chapter_number', label: 'Chapter Number' },
  { id: 'number_of_lessons', label: 'No of Lessons ' },
  { id: 'mcq_count', label: 'MCQs Count ' },
...
export default function ChaptersListView() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      request.get(
        `backoffice/chapter?page=${pagination.page}&page_size=${pagination.page_size}`
      ),
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.chapters.edit(id));
    },
    [router]
  );

  const handleDeleteRow = async (id) => {
    const response = await request.delete(`backoffice/chapter/${id}`);
    ...
    <Container maxWidth="lg">
      <Box sx={{ position: 'relative', mb: { xs: 3, md: 5 } }}>
        ...
        />
          ...
          href={paths.dashboard.chapters.new}
          variant="contained"
