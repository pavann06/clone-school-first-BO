...
import OfflineCoursesTableRow from '../offlinecourses-table-row';
...
const TABLE_HEAD = [
  ...
  { id: 'final_course_fee', label:'Final Course fee'},
 
  { id: 'actions ', label: 'Actions' },
];

export default function OfflineCoursesListView() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return request.get(
        `backoffice/online/course?page=${pagination.page}&page_size=${pagination.page_size}${categoryFilter}${subcategoryFilter}`
      );
    },
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.onlinecourses.edit(id));
    },
    [router]
  );

  const handleDeleteRow = async (id) => {
    const response = await request.delete(`backoffice/online/course/${id}`);
    ...
    <Container maxWidth="lg">
      <Box sx={{ position: 'relative', mb: { xs: 3, md: 5 } }}>
        ...
        <Button
          ...
          href={paths.dashboard.onlinecourses.new}
          variant="contained"
