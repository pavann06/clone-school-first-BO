...
//         loading={loading}
//         value={categories.filter((category) => value.includes(category.display_name)) || []}
//         onInputChange={(event, newInputValue) => setSearchTerm(newInputValue)}
//         onChange={(event, newValue) => onChange(newValue.map((item) => item.display_name))}
//         renderInput={(params) => (
//           <TextField {...params} label="Search Categories" variant="outlined" fullWidth />
//         )}
//       />
//     </Box>
//   );
...
const CategoriesDropdown = ({ value, onChange }) => {
  const { enqueueSnackbar } = useSnackbar();
  const [categories, setCategories] = useState([]); // Current search results
  const [selectedCategories, setSelectedCategories] = useState([]); // Persist selected items
  const [loading, setLoading] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
    ...
    <Box>
      <Autocomplete
        ...
        onChange={(event, newValue) => {
          setSelectedCategories(newValue); // Persist selected categories
          onChange(newValue.map((item) => item.display_name)); // Update parent component
        }}
        renderInput={(params) => (
          <TextField {...params} label="Search Categories" variant="outlined" fullWidth />
        )}
