...
import FeelsTableRow from '../feels-table-row';
...
export default function FeelsListView() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      request.get(`backoffice/feels?page=${pagination.page}&page_size=${pagination.page_size}`),
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.feels.edit(id));
    },
    [router]
  );

  const handleDeleteRow = async (id) => {
    const response = await request.delete(`backoffice/feels/${id}`);
    ...
    <Container maxWidth="lg">
      <Box sx={{ position: 'relative', mb: { xs: 3, md: 5 } }}>
        ...
        <Button
          ...
          href={paths.dashboard.feels.new}
          variant="contained"
