...
import EdutainmentTableRow from '../word-table-row';
...
export default function WordListView() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      request.get(
        `backoffice/word?page=${pagination.page}&page_size=${pagination.page_size}`
      ),
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.word.edit(id));
    },
    [router]
  );

  const handleDeleteRow = async (id) => {
    const response = await request.delete(`backoffice/word/${id}`);
