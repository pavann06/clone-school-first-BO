...
import WebinarTableRow from '../webinar-table-row';
...
export default function WebinarListView() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      request.get(
        `backoffice/webinar?page=${pagination.page}&page_size=${pagination.page_size}`
      ),
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.webinar.edit(id));
    },
    [router]
  );

  const handleDeleteRow = async (id) => {
    const response = await request.delete(`backoffice/webinar/${id}`);
    ...
    <Container maxWidth="lg">
      <Box sx={{ position: 'relative', mb: { xs: 3, md: 5 } }}>
        ...
        />
          ...
          href={paths.dashboard.webinar.new}
          variant="contained"
