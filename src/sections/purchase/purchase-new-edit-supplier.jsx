...
export default function PurchaseNewEditSupplier() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['contacts', 'supplier', filters],
  ...
  const fetchSupplier = async (id) => {
    ...
    setSelectedSupplier(response?.info?.[0]);
  };

  useEffect(() => {
    if (supplier) {
      fetchSupplier(supplier);
    }
  }, [supplier]);
  ...
  return (
    <Stack sx={{ width: 1 }}>
      ...
      {selectedSupplier && (
        <Stack spacing={1}>
          <>
            <Typography variant="subtitle2">{selectedSupplier.full_name}</Typography>
            <Typography variant="body2">{selectedSupplier.mobile}</Typography>
            <Typography variant="body2">{selectedSupplier.address1}</Typography>
          </>
        </Stack>
      )}
    </Stack>
