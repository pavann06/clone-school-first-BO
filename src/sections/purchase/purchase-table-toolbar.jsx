...
export default function PurchaseTableToolbar({
  ...
  const handleFilterSupplier = useRef(
    debounce((event) => {
      setFilters((prev) => ({
        ...prev,
        offset: 0,
        supplier_name: event.target.value.toLowerCase(),
      }));
    }, 750)
  ).current;

  const handleFilterOrganizer = useRef(
    debounce((event) => {
      setFilters((prev) => ({
        ...prev,
        offset: 0,
        organizer_name: event.target.value.toLowerCase(),
      }));
    }, 750)
  ).current;

  const handleReceiptNo = useRef(
    debounce((event) => {
      setFilters((prev) => ({
        ...prev,
        offset: 0,
            ...
            >
              ...
              <Stack direction="row" alignItems="center" spacing={2} flexGrow={1} sx={{ width: 1 }}>
                <TextField
                  fullWidth
                  value={filters.name}
                  onChange={handleFilterOrganizer}
