...
export default function SurveyQuestionEditForm({ surveyId }) {
  ...
  const SurveyQuestionSchema = Yup.object().shape({
    ...
    question: Yup.string().required('Question Text is required'),
    options: Yup.array()
      .when('question_type', (question_type, schema) => 
        (question_type === 'Single Choice' || question_type === 'Multiple Choice') 
          ? schema.min(1, 'At least one option is required')
          : schema.strip()
      ),
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (data.question_type === 'Yes/No') {
        ...
        payload.options = data.options.map((opt) => opt.value).filter((val) => val.trim() !== '');
      } else {
        payload.options = [];
      }

      const response = await CreateSurveyQuestion(payload);
  ...
  render={({ field: inputField }) => <RHFTextField {...inputField} label={`Option ${index + 1}`} />}
/>
