...
export default function SurveyQuestionEditForm({ surveyId }) {
  ...
  //       payload.options = Array.isArray(data.options) ? data.options.filter((option) => option.trim() !== '') : [];
  //     } else {
  //       payload.options = [];
  //     }
  ...
  //       enqueueSnackbar(response.message || 'Survey question created successfully!', { variant: 'success' });
        ...
        <Grid xs={12} md={8}>
          <Card>
            <Stack spacing={3} sx={{ p: 3 }}>
              ...
              >
                <RHFSelect name="question_type" label="Question Type" onChange={handleQuestionTypeChange}>
