...
export default function SurveyListView() {
  ...
  const [totalCount, setTotalCount] = useState(0);
  const [pagination, setPagination] = useState({ page: 1, page_size: 10 });
  const [status, setStatus] = useState('draft');  // Added state for status

  const { data, isLoading } = useQuery({
    queryKey: ['survey', pagination.page, pagination.page_size, status],
    queryFn: () =>
      request.get(`backoffice/survey?page=${pagination.page}&page_size=${pagination.page_size}&status=${status}`),
    keepPreviousData: true,
    onError: (err) => {
      enqueueSnackbar(`Error fetching data: ${err.message}`, { variant: 'error' });
    },
