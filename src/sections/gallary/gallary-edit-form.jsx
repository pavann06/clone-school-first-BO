...
import { Box, Card, Stack, MenuItem, Typography, Grid } from '@mui/material';
import LoadingButton from '@mui/lab/LoadingButton';
...
export default function GallaryNewEditForm({ currentEdutainment }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      images: parseArrayField(currentEdutainment?.images),
      thumbnail_images: parseArrayField(currentEdutainment?.thumbnail_images),
      school_id: currentEdutainment?.school_id || '',
    }),
    [currentEdutainment]
  );
    ...
    async (acceptedFiles) => {
      ...
      try {
        ...
        if (validUrls.length > 0) {
          ...
          enqueueSnackbar(`${validUrls.length} image(s) uploaded successfully`, {
            variant: 'success',
          });
        }
      } catch (error) {
                    ...
                    {values.images.map((url) => (
                      ...
                      >
                        ...
                        <Box
                          onClick={() => {
                            const updatedImages = values.images.filter((img) => img !== url);
                            const updatedThumbnails = values.thumbnail_images.filter(
                              (thumb) => thumb !== url
                            );
                            setValue('images', updatedImages, { shouldValidate: true });
                            setValue('thumbnail_images', updatedThumbnails, {
                              shouldValidate: true,
                            });
                          }}
                          sx={{
