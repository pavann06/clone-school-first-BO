...
export default function StudentsListView() {
  ...
  // Fetch students with school_id (if any)
  const { data, isLoading } = useQuery({
    queryKey: ['students', pagination.page, pagination.page_size, schoolId],
    queryFn: () => {
      const base = `backoffice/student?page=${pagination.page}&page_size=${pagination.page_size}`;
      const url = schoolId ? `${base}&school_id=${schoolId}` : base;
